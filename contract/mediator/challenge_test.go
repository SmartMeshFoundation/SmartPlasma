...
//	uid := testDeposit(t, i)
//
//	txGood := testTx(t, zero, uid, one, user1.From, user1)
//
...
//	}
//
//	txBad := testTx(t, one, uid, one, user1.From, user1)
//
...
//	}
//
//	ethTX4, err := i.rootUser2Session.ChallengeExit(uid, rawBadTx,
//		proof2, two)
//	if err != nil {
//		t.Fatal(err)
//	}
...
func TestChallengeDoubleSpending(t *testing.T) {
	...
	txGood2 := testTx(t, one, uid, one, user2.From, user1)
	...
	txBad := testTx(t, one, uid, one, owner.From, user1)
...
func TestEarlyChallengeDoubleSpending(t *testing.T) {
	...
	// tx1
	tx1Good := testTx(t, zero, uid, one, user2.From, user1)
	...
	// tx2
	tx2Good := testTx(t, one, uid, one, owner.From, user2)
	...
	// tx3 - bad transaction
	tx3Bad := testTx(t, two, uid, one, user1.From, user2)
	...
	// tx4
	tx4 := testTx(t, three, uid, one, owner.From, user1)
	...
	// tx5
	tx5 := testTx(t, four, uid, one, user2.From, owner)
